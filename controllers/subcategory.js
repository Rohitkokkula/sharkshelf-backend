...
const put = async (req, res, next) => crud.update(req,res,next,prisma.subcategory,required,include);

const post = async (req, res, next) => {
    let { name,category_id } = req.body;
    let required = { name,category_id };
    crud.create(req,res,next,prisma.subcategory,required,include,{name});
}
